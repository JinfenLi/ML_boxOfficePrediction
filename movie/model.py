...
def gen_train_data(data, train_size = 190, out_dim = 5, use17=False):
    ...
    if use17:
        ...
        return np.asarray(train_data), np.asarray(test_data)[:, 1:], np.asarray(test_label)
    else:
...
class MLP(object):
    def __init__(self, sess, name="default", input_dim=20, output_dim=10, hidden_depth=5, stddev=0.02, learning_rate=0.008, log_dir='logs', data_dir='box_office.xlsx', scope=None):
        self.sess = sess
        self.name = name
    ...
    def get_data(self, train_size=190, use17=False):
        if not use17:
            ...
            self.type_dict = build_dict(type_, 0, 2)
            tran_dir_act_by_rank(data=self.data, rank_dict=self.rank_dict, type_dict=self.type_dict,
                                 director_col=12, actor_col=13, type_col=14)
            self.train_data_label, self.test_data, self.test_label = gen_train_data(data=self.data,
                                                                                    train_size=train_size,
                                                                                    out_dim=self.output_dim,
                                                                                    use17=True)
    ...
    def build_MLP(self):
        ...
        with tf.name_scope(name=self.name + "_output_layer"):
            loss = tf.reduce_mean(-tf.reduce_sum(y * tf.log(y_hat), reduction_indices=[1]), name=self.name + "_cross_entropy")
        ...
        with tf.name_scope(name=self.name + "_predict"):
            predict = tf.equal(tf.argmax(y, 1), tf.argmax(y_hat, 1), name=self.name + "_predict")
    ...
    def run(self, steps=20000, init_=True, name="", train_size=190, use17=False):
        ...
        for i in range(steps):
            ...
            if i % 50 == 0:
                ...
                writer.add_summary(self.sess.run(merged, feed_dict={self.x: self.test_data, self.y: self.test_label}), i)
                acc_ = acc
                if acc > max_:
                    max_ = acc
                    if not os.path.exists("models/" + self.name + "/" + name):
                        os.mkdir("models/" + self.name + "/" + name)
                    saver.save(self.sess, "models/" + self.name + "/" + name + "/" + "epoch_" + str(i))
...
if __name__ == "__main__":
    ...
    a = [0.55555558, 0.33333334, 0.33333334, 0.5, 0.22222222, 0.22222222, 0.3888889, 0.22222222, 0.33333334, 0.33333334, 0.27777779, 0.11111111, 0.22222222, 0.16666667, 0.33333334, 0.27777779, 0.3888889, 0.22222222, 0.33333334, 0.27777779]
